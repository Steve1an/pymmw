...
def plot_buffer(fig, idx, xy, a, k, n, b):
    ...
    ss = [np.fft.fft(chirp[i]) for i in range(a)]
    ...
    if idx == l:
        ...
        for i in range(len(ss)):
            ax.plot(rng, np.abs(ss[i])*win, linewidth=1.0, c=col[i], alpha=1.0 / len(ss))
        ax.relim()
        ax.autoscale_view()
